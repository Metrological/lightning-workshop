...
function startApp() {
  if (ux.Ui.hasOption("useInterval")) {
    console.log('use interval instead of request animation frame'); // Work-around for requestAnimationFrame bug.
  ...
  var fontFaces = [new FontFace('RobotoRegular', 'url(./fonts/roboto-regular.ttf)')];
  Promise.all(fontFaces.map(function (ff) {
    ...
    searchParams.forEach(function (value, key) {
      if (key.startsWith("key_")) {
        var number = parseInt(key.substr(4));

        if (!isNaN(number)) {
          navigationKeys[number] = value;
        }
      }
    });
    var memoryPressure = parseInt(ux.Ui.getOption('memoryPressure')) || 16e6;
    console.log('GPU memory pressure: ' + memoryPressure);
    var options = {
      stage: {
        w: 1920,
        h: 1080,
        clearColor: ux.Ui.getOption('transparentBg') ? 0x00000000 : 0xFF000000,
    ...
    try {
      ...
      bootstrap.startApp(new com_metrological_app_ARD(bootstrap));
    } catch (e) {
      alert("error " + e);
    }
...
function isSupportingES6() {
  ...
  var features = ["const foo = (test = 1) => {};", "class Temp {get a() {return 1;}}; class Temp2 extends Temp {get b() {return 2;}};", "const template = `bka ${1}\n${2}`;", "let t = 1;", "const data = {a: 1, n: 2}; const {a, b} = data;", "const c = 1; const data2 = {c};"];
...
var folder = isSupportingES6() ? "src" : "src.es5";
...
loadJsFile("lightning-web.js").then(function () {
  return loadJsFile("ux.js").then(function () {
    return Promise.all([loadJsFile("com.metrological.app.ARD.js")]);
  });
}).catch(function (e) {
  console.error(e);
}).then(function () {
