    ...
    Promise.all(fontFaces.map(function(ff) {
        ...
        searchParams.forEach((value, key) => {
            if (key.startsWith("key_")) {
                const number = parseInt(key.substr(4));
                if (!isNaN(number)) {
        ...
        try {
            ...
            bootstrap.startApp(new com_metrological_app_TuneIn(bootstrap));
        } catch (e) {
            alert("error " + e)
        }
...
loadJsFile("lightning-web.js").then(function() {
    return loadJsFile("ux.js").then(function() {
        return Promise.all([
            loadJsFile("com.metrological.app.TuneIn.js")
        ]);
    })
}).catch(function(e) {
    console.error(e);
